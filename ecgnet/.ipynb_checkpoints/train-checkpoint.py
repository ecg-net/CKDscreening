...
def train(
    ...
    bs = 20
    train_ds = EchoECG(root=root,csv=csv,split='Train',target_type='ICU_ADMIT_TIME')
    validation_ds = EchoECG(root=root,csv=csv,split='Val',target_type = 'ICU_ADMIT_TIME')

    train_dataloader = torch.utils.data.DataLoader(train_ds, batch_size=bs,num_workers=48)
    val_dataloader=torch.utils.data.DataLoader(validation_ds, batch_size=bs,num_workers=48)
    ...
    model = build_model(model_type, 1, 1 if is_2d else 12, is_2d=is_2d, binary=binary, batch_norm=batch_norm, default_conv_kernel=conv_width, drop_prob=drop_prob,
                         binary_cutoff=binary_cutoff, first_layer_out_channels=first_layer_out_channels, first_layer_kernel_size=first_layer_kernel_size).float()
...
def run_epoch(split, loader, model, optim, epoch, binary, f, device, cutoff=None, log=True):
    ...
    for (x, y) in tqdm.tqdm(loader):
        ...
        #x = x.to(device)
        y = y.to(device)
        (y, yh, loss) = model.module.train_step(x, y.long())
        #print(y,yh)
        if split=='train':
            optim.zero_grad()
            loss.backward()
